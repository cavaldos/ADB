...
function generateSQL(procedureName, data, count) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        for (const key in data) {
            ...
            } else if (data[key] === 'SchoolYear') {
                fakeData[key] = faker.date.past().getFullYear().toString();
            }
        }
        ...
        // Tạo câu lệnh SQL
        const sql = `EXEC ${procedureName} '${fakeData.UserName}', '${fakeData.Password}', '${fakeData.Email}', '${fakeData.FullName}', '${fakeData.Phone}', '${fakeData.Address}', '${fakeData.SchoolYear}';`;
...
// Tên procedure
const procedureName = 'create_student';
...
const filePath = path.join(dir, 'studentData.sql');
