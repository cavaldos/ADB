...
function generateSQL(numLessons, procedureName) {
    ...
    for (let i = 1; i <= numLessons; i++) {
        ...
        const courseID = getRandomInt(1111,1112); // Giả định ID khóa học nằm trong khoảng từ 1001 đến 1990
        const topicID = getRandomInt(534, 564); // Giả định ID chủ đề nằm trong khoảng từ 1001 đến 1990
...
const numLessons = 50; // Số lượng bài học
const procedureName = 'create_lesson_test'; // Tên thủ tục
...
const filePath = path.join(dir, 'generated_lesson_tests.sql');
fs.writeFile(filePath, sql, (err) => {
    if (err) throw err;
    console.log('Tệp generated_lesson_tests.sql đã được tạo trong thư mục fakedata!');
});
