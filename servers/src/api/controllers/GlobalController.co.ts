...
import ForumRepo from "../../repositories/forum.repo";
...
const GlobalController = {
  ...
  Banking: {
    ...
    async getHistoryBanking(req: Request, res: Response) {
      try {
        const { bankAccountID } = req.body;
        const result = await BankAccountRepo.getHistoryBanking(bankAccountID);
        return res.status(200).json({
          message: `Get history banking`,
          status: 200,
          data: result,
        });
      } catch (error: any) {
  ...
  Forum: {
    async createForum(req: Request, res: Response) {
      try {
        const { courseID } = req.body;
        await ForumRepo.createDiscussionForum(courseID);
        return res.status(200).json({
          message: `Created forum by UserID: ${courseID}`,
          status: 200,
        });
      } catch (error: any) {
        return res.status(500).json({
    ...
    async createMess(req: Request, res: Response) {
      try {
        ...
        return res.status(200).json({
          message: `Created message by UserID: ${userID}`,
          status: 200,
        });
      } catch (error: any) {
        return res.status(500).json({
    ...
    async deleteDiscussionForum(req: Request, res: Response) {
      try {
        const { DiscussionForumID, forumMessageID, userID } = req.body;
        await ForumRepo.deleteDiscussionForum(
          DiscussionForumID,
          forumMessageID,
          userID
        );
        return res.status(200).json({
          message: `Deleted discussion forum by UserID: ${userID}`,
          status: 200,
        });
      } catch (error: any) {
        return res.status(500).json({
    ...
    async getAllMessageForum (req: Request, res: Response) {
      try {
        const { courseID } = req.body;
        const result = await ForumRepo.getAllMessageForum(courseID);
        return res.status(200).json({
          message: `Get all message forum`,
          status: 200,
          data: result,
        });
      } catch (error: any) {
